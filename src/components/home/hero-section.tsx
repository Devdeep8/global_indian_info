    ...
    <div className="relative w-full group">
            ...
            <CarouselItem key={item.id}>
              <div className="relative w-full h-[70vh] min-h-[500px]">
                ...
                <div className="absolute inset-0 overflow-hidden">
                  <motion.img
                    initial={{ scale: 1 }}
                    animate={{ scale: index === currentIndex ? 1.05 : 1 }}
                          ...
                          >
                            <div className="space-y-4">
                              {item.category && (
                                <motion.div 
                                  ...
                                  className="inline-flex items-center px-3 py-1 rounded-full bg-primary/20 border border-primary/30 backdrop-blur-sm"
                                >
                            ...
                            {item.excerpt && (
                              <motion.p 
                                ...
                                className="text-lg md:text-xl text-gray-200 max-w-xl leading-relaxed line-clamp-3"
                              >
                                {item.excerpt}
                              </motion.p>
                            )}
                            ...
                            <motion.div 
                              ...
                              className="flex items-center space-x-6 text-sm text-gray-300 pt-2"
                            >
                              {item.author && (
                                <div className="flex items-center gap-2">
                                  <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center border border-white/20">
                                    {item.author.avatarUrl ? (
                                      <img src={item.author.avatarUrl} alt={item.author.name} className="w-full h-full rounded-full object-cover" />
                                    ) : (
                            ...
                            >
                              <Link href={`/articles/${item.slug}`}>
                                <Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-full px-8 py-6 text-lg shadow-lg hover:shadow-primary/25 transition-all duration-300">
                                  Read Full Story
                                </Button>
                              </Link>
                            </motion.div>
      ...
      <div className="absolute inset-y-0 left-0 flex items-center pl-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <Button
          variant="ghost"
          size="icon"
          className="bg-black/20 hover:bg-black/40 text-white rounded-full w-12 h-12 backdrop-blur-sm border border-white/10"
